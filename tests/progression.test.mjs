...
game.cam.y = 0;
game.clampCamera();
assert.equal(game.cam.x, 640, 'camera clamps to the left world edge at 1280px');
assert.equal(game.cam.y, 360, 'camera clamps to the top world edge at 720px');
game.cam.x = 640.1;
const snappedGroundX = game.worldToScreen(10 * config.WORLD.tile, 0).x;
game.cam.x = 640.49;
assert.equal(game.worldToScreen(10 * config.WORLD.tile, 0).x, snappedGroundX, 'sub-pixel camera easing does not make ground texture swim');
game.loadMap('field', 3, 14);
assert.equal(game.cam.x, 640, 'map entry cannot expose space beyond the left world edge');
assert.equal(game.worldToScreen(game.player.x, game.player.y).x, 144, 'edge entry player stays at its world-space screen position');
game.loadMap('bich');
